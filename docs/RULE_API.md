...
- 单个权重必须在 0 到 1 之间；
- 权重总和不得超过 1；
- 未返回的证券不参与本次调仓，持有数量保持不变；
- 显式权重 `0` 表示清仓对应证券；
- 空字典 `{}` 等同 `NO_REBALANCE`，不创建 D+1 目标。

映射只列出需要调整的证券，但每个权重仍是相对 D+1 总资产的绝对目标权重，不是权重增量
或买卖数量。框架只为这些显式证券比较目标和实际持仓后生成订单。全部清仓必须为所有需要
退出的持仓显式返回零权重。
...
表示本次不创建新的 D+1 目标，维持现有持仓数量。Rule 返回 `{}` 时框架会将其规范化为
同一结果。
